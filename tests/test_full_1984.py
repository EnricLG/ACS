...
- Phase 0: Preprocessing (char grid)
- Phase 1: Rotations (char grid)
- Phase 2: Dictionary cipher (char grid -> char grid)
- Phase 3: Pairwise linear transform (convert char grid to integers, then transform, then visual mapping)
...
# Full 1984 text (ensure it's the complete version)
...
print("=" * 70)
print("Full encryption pipeline for 1984 text (Phases 0, 1, 2, 3)")
print("=" * 70)
...
# Phase 2: Dictionary cipher (character grid -> character grid)
print("\n🔷 Phase 2: Dictionary cipher...")
cipher2 = Phase2DictCipher(master_key, iv)
start = time.time()
grid2 = cipher2.encrypt(grid1)   # grid2 is a grid of characters
print(f"   Done in {time.time()-start:.2f}s.")
...
# Convert characters to integers (0-99) for Phase 3
grid2_ints = [[ALPHABET.index(ch) for ch in row] for row in grid2]

# Phase 3: Pairwise linear transform (integers -> integers 0-9999)
...
# Phase 3: Visual mapping (Chinese characters / CSS colors)
print("\n🔷 Phase 3: Visual mapping...")
visual = Phase3Visual(seed=b'1984')
start = time.time()
grid3_visual = []
for row in grid3_numeric:
    visual_row = [visual.to_visual(w) for w in row]
    grid3_visual.append(visual_row)
print(f"   Done in {time.time()-start:.2f}s.")
with open("output_1984_phase3_visual.txt", "w") as f:
    f.write("Phase 3: Visual representation (Chinese characters / CSS colors)\n")
    f.write("=" * 70 + "\n")
    for i in range(100):
        # Write each cell's visual representation (variable length)
...
print("   - output_1984_phase3_visual.txt")