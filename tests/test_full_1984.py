...
# Visual mapping after Phase 4
print("\n🔷 Visual mapping (Phase 3 style) on Phase 4 output...")
visual = Phase3Visual(seed=b'1984')
start = time.time()
grid4_visual = []
for row in grid4_numeric:
    grid4_visual.append([visual.to_visual(w) for w in row])
print(f"   Done in {time.time()-start:.2f}s.")

# Save text version of visual output
with open("output_1984_phase4_visual.txt", "w", encoding="utf-8") as f:
    f.write("Phase 4: Visual representation (Chinese characters / CSS colors)\n")
    f.write("=" * 70 + "\n")
    for i in range(100):
        f.write("Row {:3d}: ".format(i) + " ".join(grid4_visual[i]) + "\n")
    f.write(f"\nSeed: {seed0.hex()}\nMaster key: {master_key.hex()[:16]}...\nIV: {iv.hex()}\n")

# Save HTML version of visual output
with open("output_1984_phase4_visual.html", "w", encoding="utf-8") as f:
    f.write(visual.to_html(grid4_visual))
print("   - output_1984_phase4_visual.html")
...
print("   - output_1984_phase4_visual.txt")
print("   - output_1984_phase4_visual.html")