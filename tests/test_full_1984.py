...
- Visual outputs: Phase 3 uses colors only; Phase 4 uses exotic characters only.
"""
...
# Visual mapping after Phase 3 (colors only)
print("\n🔷 Visual mapping (Phase 3, colors only)...")
visual_colors = Phase3Visual(seed=b'1984', mode='colors')
start = time.time()
grid3_visual_colors = [[visual_colors.to_visual(w) for w in row] for row in grid3_numeric]
print(f"   Done in {time.time()-start:.2f}s.")

# Save text version (colors)
with open("output_1984_phase3_visual_colors.txt", "w", encoding="utf-8") as f:
    f.write("Phase 3: Visual representation (colors only)\n")
    f.write("=" * 70 + "\n")
    for i in range(100):
        f.write("Row {:3d}: ".format(i) + " ".join(grid3_visual_colors[i]) + "\n")
    f.write(f"\nSeed: {seed0.hex()}\nMaster key: {master_key.hex()[:16]}...\nIV: {iv.hex()}\n")

# Save HTML version (colors)
with open("output_1984_phase3_visual_colors.html", "w", encoding="utf-8") as f:
    f.write(visual_colors.to_html(grid3_visual_colors))
print("   - output_1984_phase3_visual_colors.html")
...
# Visual mapping after Phase 4 (exotic characters only)
print("\n🔷 Visual mapping (Phase 4, exotic characters only)...")
visual_chars = Phase3Visual(seed=b'1984', mode='chars')
start = time.time()
grid4_visual_chars = [[visual_chars.to_visual(w) for w in row] for row in grid4_numeric]
print(f"   Done in {time.time()-start:.2f}s.")

# Save text version (characters)
with open("output_1984_phase4_visual_chars.txt", "w", encoding="utf-8") as f:
    f.write("Phase 4: Visual representation (exotic characters only)\n")
    f.write("=" * 70 + "\n")
    for i in range(100):
        f.write("Row {:3d}: ".format(i) + " ".join(grid4_visual_chars[i]) + "\n")
    f.write(f"\nSeed: {seed0.hex()}\nMaster key: {master_key.hex()[:16]}...\nIV: {iv.hex()}\n")

# Save HTML version (characters)
with open("output_1984_phase4_visual_chars.html", "w", encoding="utf-8") as f:
    f.write(visual_chars.to_html(grid4_visual_chars))
print("   - output_1984_phase4_visual_chars.html")
...
print("   - output_1984_phase3_visual_colors.txt")
print("   - output_1984_phase3_visual_colors.html")
print("   - output_1984_phase4_numeric.txt")
print("   - output_1984_phase4_visual_chars.txt")
print("   - output_1984_phase4_visual_chars.html")